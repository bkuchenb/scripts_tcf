...
admin_page = browser.get('http://marketplace.beckett.com/admin')
#Get the login form.
login_form = admin_page.soup.select('#loginFrm')
#Find the loginEmail text box.
loginEmail = login_form[0].select('#loginEmail')
#Enter loginEmail.
loginEmail[0]['value'] = ('dialcard@nycap.rr.com')

#Find the loginPassword text box.
password = login_form[0].select('#loginPassword')
#Enter the loginPassword.
password[0]['value'] = ('ruthmick')
#Submit the form.
browser.submit(login_form[0], admin_page.url)
#*****************************************************************************#
#Get the sport.
sport = 'baseball'
...
for row in results:
    try:
        #Open the set page.
        item_management_page = browser.get(item_management_str)
        print(item_management_page.soup.title.text)
        #Find the form that contains the search box.
        search_form = item_management_page.soup.select('#refine-search')
        #Find the search box.
        search_box = search_form[0].select('#search_text')
        #Search for the set.
        search_box[0]['value'] = row[1] + ' ' + row[2]
        #Submit the form.
