...
session.auth = ('dialcard@nycap.rr.com', 'ruthmick')
...
cnx_2.commit()
try:
    year_input = 1960#input('Enter the year: ')
...
#Login in to Beckett.
response = session.post('https://marketplace.beckett.com/login/',
                 data = {'email': 'dialcard@nycap.rr.com',
                 'password': 'ruthmick'})
#Parse the server response.
page = html.parse(response.text)
#Print the page title.
#print(page.xpath('//title/text()'))
#*****************************************************************************#
#Get the sport.
sport = 'baseball'
#Build the Item Management page string.
item_management_str = ('http://marketplace.beckett.com/admin/accelerator')
item_management_str += ('/sets/search#sport=')
...
for row in results:
    try:
        #Build the Item Management page string.
        item_management_str += ('&tmm=extended&term=')
        item_management_str += (row[1] + '+' + row[2].lower())
        item_management_str += ('&year_start=' + row[1] + '&') 
#        print(item_management_str)
#        #Search for the set.
#        r2 = session.get(item_management_str)
#        print(r2.status_code)
#        soup = BeautifulSoup(r2.text, 'lxml')
#        print(soup.title.text)
        #Find the form that contains the search box.
#        search_form = item_management_page.text.soup.select('#refine-search')
#        #Find the search box.
#        search_box = search_form[0].select('#search_text')
#        #Search for the set.
#        search_box[0]['value'] = 
#        #Submit the form.
#        grid_page = browser.submit(search_form[0], item_management_page.url)
#        print(grid_page.soup.title.text)
#        #Find the number of pages.
#        pages = grid_page.soup.select('#sp_1')
...
#            select = grid_page.soup.select("//select[@class=\"ui-pg-selbox\"]")
