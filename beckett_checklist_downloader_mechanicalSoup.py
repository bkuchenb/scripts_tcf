...
for row in results:
    try:
        ...
        #Open the set page.
        search_results_page = browser.get(item_management_str)
        print(item_management_str)
        print(search_results_page.soup.title.text)
        #Find the number of pages.
        pages = search_results_page.soup.select('#sp_1')
        print(pages[0]['value'])
        ...
        if pages[0].text != '1':
            ...
            select = search_results_page.soup.select('.ui-pg-selbox')
            #Save all the drop down box options.
            all_options = search_results_page.soup.select('option')
            #Choose to display 100 records.
            all_options[3].click()
        #Find all the check boxes.
        check_boxes = search_results_page.soup.select('.cbox')
